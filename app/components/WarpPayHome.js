...
export default function WarpPayHome({
  ...
  return (
    <div className="min-h-screen bg-[#0f0d14] text-white px-4 py-6 flex flex-col items-center">
      ...
      <p className="text-sm text-gray-400 mb-6">Send, request & drop tokens on Warpcast</p>
      <p className="text-sm text-gray-400 mb-6">Under construction</p>

      <div className="grid grid-cols-2 gap-4 w-full max-w-sm">
        <ActionButton icon={<FiSend />} label="Send" onClick={() => onAction('send')} />
        <ActionButton icon={<FiDollarSign />} label="Request" onClick={() => onAction('request')} />
        <ActionButton icon={<FiGift />} label="Airdrop" onClick={() => onAction('airdrop')} />
        <ActionButton icon={<FiClock />} label="History" onClick={() => onAction('history')} />
      </div>
